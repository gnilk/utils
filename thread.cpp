...
{
	iState = Thread::kThreadStateRunning;// GOAT_THREAD_STATE_RUNNING;
	iExit = pFunc(pArg);
	iState = Thread::kThreadStateStopped; //GOAT_THREAD_STATE_STOPPED;
...
Thread::Thread(GOATTHREADFUNC pFunc, void *pArg, kThreadCreateFlags flags)
{
	int err = 0;
	int win32Flags =0;
	...
	this->pFunc = pFunc;
	this->pArg = pArg;
	//	if (flags & GOAT_THREAD_CREATE_SUSPENDED) win32Flags |= CREATE_SUSPENDED;
	...
	if (hThread != NULL)
	{
		...
		{
			iState = Thread::kThreadStateSuspended;//  GOAT_THREAD_STATE_SUSPENDED;
		}
	} else
	{
		err = GOAT_THREAD_ERR_CREATIONFAILED;
	}
	// Last error ??
	// return err;
}
...
Thread::~Thread()
{
	if (iState != Thread::kThreadStateStopped) // GOAT_THREAD_STATE_STOPPED)
	{
		Stop();
	}
	CloseHandle(hThread);			
...
// static
Thread *Thread::Create(GOATTHREADFUNC pFunc, void *pArg, kThreadCreateFlags flags)
{
...
int Thread::Start()
{
	int err = GOAT_THREAD_ERR_NOERROR;
	if (iState == Thread::kThreadStateSuspended) // GOAT_THREAD_STATE_SUSPENDED)
	{
		...
		err = GOAT_THREAD_ERR_NOERROR;
	} else
	{
		err = GOAT_THREAD_ERR_WRONGSTATE;
	}
	return err;
}
...
int Thread::Stop()
{
	int err = GOAT_THREAD_ERR_INVALIDTHREAD;
	if (iState == Thread::kThreadStateRunning) // GOAT_THREAD_STATE_RUNNING)
	{
...
int Thread::Yield()
{
	...
	if (!SwitchToThread())
	    Sleep(1);
	return GOAT_THREAD_ERR_NOERROR;
}
...
Thread::Thread(GOATTHREADFUNC pFunc, void *pArg, kThreadCreateFlags flags)
{
	...
	if (!pthread_create(&hThread, &attr, funcThread_callback, this)) err = GOAT_THREAD_ERR_CREATIONFAILED;
...
int Thread::Stop()
{
	int err = GOAT_THREAD_ERR_NOTSUPPORTED;
	return err;
}
