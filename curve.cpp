 ...
 * Hermite
 ...
 - Rewrite load/save to use proper goat I/O handling
 </pre>
 ...
 - 15.10.09, FKling, Imported into Goat from Yapt2
 ---------------------------------------------------------------------------*/
#include <math.h>
#include <vector>
#include <assert.h>
...
static const char *classNameArray[]=
{
"Unknown",
"Hold",
"Linear",
...
void KBSpline::Interpolate(double t, int idxStart, int idxEnd, double *dest)
{
	...
	std::vector<double> &ch1 = keys[idxStart]->values;
	std::vector<double> &ch2 = keys[idxStart+1]->values;
	// ch4 = dest
	...
	else
	{
		if(idxStart != 0)
		{
			std::vector<double> &ch0 = keys[idxStart-1]->values;
		...
		{
			std::vector<double> &ch3 = keys[idxStart+2]->values;
